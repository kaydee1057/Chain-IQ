...
class ProductionDatabase {
    ...
    private function __construct() {
        try {
            // Use SQLite with production optimizations
            $this->pdo = new PDO('sqlite:' . __DIR__ . '/../admin_app.db');
    ...
    private function createProductionSchema() {
        $queries = [
            // USERS - Production schema
            ...
            "CREATE TABLE IF NOT EXISTS balances (
                ...
                locked_balance DECIMAL(36,8) NOT NULL DEFAULT 0,
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
                UNIQUE(user_id, asset)
            )",
            
            // TRANSACTIONS - Production transaction table
            ...
            "CREATE TABLE IF NOT EXISTS cards (
                ...
                status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active','frozen','cancelled','expired')),
    ...
    private function createDemoCard($userId) {
        ...
        $stmt = $this->pdo->prepare("INSERT OR IGNORE INTO cards (user_id, type, card_number, cvv, expiry_month, expiry_year, cardholder_name, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$userId, 'platinum', $cardNumber, $cvv, $expiryMonth, $expiryYear, 'Demo User', 'active']);
    }
    ...
    // Get user balance with lock for updates
    ...
    public function updateBalance($userId, $asset, $amount, $type = 'credit') {
        return $this->executeWithTransaction(function($pdo) use ($userId, $asset, $amount, $type) {
            ...
            $stmt = $pdo->prepare("SELECT balance FROM balances WHERE user_id = ? AND asset = ?");
            $stmt->execute([$userId, $asset]);
